...
PrintDialog::PrintDialog(KBanking *app,
                         const char *docTitle,
                         const char *docType,
                         const char *descr,
                         const char *text,
                         ...
                         bool modal,
                         WFlags fl)
:PrintDialogUi(parent, name, modal, fl)
,_banking(app)
,_docTitle(docTitle)
,_docType(docType)
,_descr(descr)
...
void PrintDialog::loadPrinterSetup(){
  GWEN_DB_NODE *db;
  GWEN_BUFFER *dbuf;
  const char *s;
...
void PrintDialog::savePrinterSetup() {
  GWEN_DB_NODE *db;
  GWEN_BUFFER *dbuf;
  const char *s;
  uint top, left, bottom, right;
  ...
  if (!_fontFamily.isEmpty())
    GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_OVERWRITE_VARS,
                         "FontFamily",
                         KBanking::QStringToUtf8String(_fontFamily).c_str());
  ...
  fname=KBanking::QStringToUtf8String(_printer->outputFileName());
  if (!fname.empty())
    GWEN_DB_SetCharValue(db, GWEN_DB_FLAGS_OVERWRITE_VARS,
...
void PrintDialog::slotSetup(){
  _printer->setup();
}


void PrintDialog::slotFont() {
  bool ok;
