...
#include "jobmodifydatedtransfer.h"
#include "jobmodifydatedtransfer_be.h"
...
AB_JOB *AB_JobModifyDatedTransfer_new(AB_ACCOUNT *a){
  return AB_JobTransferBase_new(AB_Job_TypeModifyDatedTransfer, a);
}
...
AB_JOB *AB_JobModifyDatedTransfer_fromDb(AB_ACCOUNT *a, GWEN_DB_NODE *db) {
  return AB_JobTransferBase_fromDb(AB_Job_TypeModifyDatedTransfer, a, db);
}
