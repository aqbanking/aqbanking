...
static int _mkDownloadInitRequest_H002(AB_PROVIDER *pro,
...
static int _mkDownloadTransferRequest_H002(AB_PROVIDER *pro,
                                           GWEN_HTTP_SESSION *sess,
                                           AB_USER *u,
...
static int _mkDownloadReceiptRequest_H002(AB_PROVIDER *pro,
                                          GWEN_HTTP_SESSION *sess,
                                          AB_USER *u,
                                          const char *transactionId,
                                          int receiptCode,
                                          ...
                                          const GWEN_DATE *toDate)
{
  ...
  rv=_mkDownloadInitRequest_H002(pro, sess, u, requestType, fromDate, toDate, &msg);
  if (rv<0) {
    DBG_INFO(AQEBICS_LOGDOMAIN, "here (%d)", rv);
    return rv;
  }
  ...
  if (1) {
    ...
    for (;;) {
      ...
      rv=_mkDownloadTransferRequest_H002(pro, sess, u, transactionId, segmentNumber, &msg);
      if (rv<0) {
        DBG_INFO(AQEBICS_LOGDOMAIN, "here (%d)", rv);
        GWEN_Buffer_free(dbuffer);
        GWEN_Crypt_Key_free(skey);
...
int _mkDownloadInitRequest_H002(AB_PROVIDER *pro,
                                GWEN_HTTP_SESSION *sess,
                                AB_USER *u,
                                const char *requestType,
                                const GWEN_DATE *fromDate,
...
int _mkDownloadReceiptRequest_H002(AB_PROVIDER *pro,
                                   GWEN_HTTP_SESSION *sess,
                                   AB_USER *u,
                                   const char *transactionId,
                                   int receiptCode,
