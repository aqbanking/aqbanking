                ...
                char **argv)
{
  ...
  if (rv<0) {
    ...
    if (rv<0) {
      DBG_ERROR(0, "Error retrieving bank parameter data (%d)", rv);
    ...
    if ((EBC_User_GetStatus(u)==EBC_UserStatus_Enabled) &&
      ...
      fprintf(stderr, "Retrieving customer information\n");
      rv1=EBC_Provider_Send_HKD(pro, u, 1);
      fprintf(stderr, "Retrieving user information\n");
      rv2=EBC_Provider_Send_HTD(pro, u, 1);
      if ((rv1<0) && (rv2<0)) {
        DBG_ERROR(0, "Could neither send HKD not HTD request (%d, %d)",
                  rv1, rv2);
        GWEN_Gui_ProgressEnd(guiid);
        return 4;
      }
      ...
      fprintf(stderr, "The bank does not support download of account information\n");
    }
    GWEN_Gui_ProgressEnd(guiid);
  }
