        ...
        const char *keyName)
{
    ...
    if ( canVerifyWithHash ) {
        ...
        if ( matchingBytes == keySize ) {
            ...
            DBG_ERROR(AQHBCI_LOGDOMAIN, "Verify new server %s key, please verify! (num: %d, version: %d, hash: %s)", keyName, keyNum, keyVer, hashString);
            ...
            GWEN_Buffer_AppendString(msgBuffer,"Could not verify the received key with the key hash stored on your banking card!\n");
