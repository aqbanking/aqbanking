...
#define AH_MSGRDH7_MAXKEYBUF 4096
static int
AH_MsgRdh7__Verify_Internal(GWEN_CRYPT_KEY *k,
...
static
GWEN_CRYPT_KEY * AH_MsgRhd7__VerifyInitialSignKey7(GWEN_CRYPT_TOKEN *ct,
...
int AH_MsgRdh7__PrepareCryptoSeg7(AH_MSG *hmsg,
				  AB_USER *u,
  ...
  if ( crypt ) {
    ...
    GWEN_DB_SetIntValue(cfg, GWEN_DB_FLAGS_DEFAULT, "key/keynum",7);
    //GWEN_Crypt_Key_GetKeyNumber(ek));
    GWEN_DB_SetIntValue(cfg, GWEN_DB_FLAGS_DEFAULT, "key/keyversion",1);
    //GWEN_Crypt_Key_GetKeyVersion(ek));
  }
  else {
    assert(ki);
    GWEN_DB_SetIntValue(cfg, GWEN_DB_FLAGS_DEFAULT, "key/keynum",7);
...
int AH_Msg_SignRdh7(AH_MSG *hmsg,
		    AB_USER *su,
		    GWEN_BUFFER *rawBuf,
		    const char *signer) {
  AH_HBCI *h;
  ...
  rv=AH_MsgRdh7__PrepareCryptoSeg7(hmsg, su, ki, cfg, 0, 1);
  if (rv) {
...
int AH_Msg_EncryptRdh7(AH_MSG *hmsg) {
  AH_HBCI *h;
  GWEN_XMLNODE *node;
  GWEN_DB_NODE *cfg;
  GWEN_BUFFER *mbuf;
  ...
  uint8_t encKey[AH_MSGRDH7_MAXKEYBUF+64];
  int encKeyLen;
  uint32_t gid;
  ...
  rv=AH_MsgRdh7__PrepareCryptoSeg7(hmsg, u, ki, cfg, 1, 0);
  if (rv) {
    DBG_INFO(AQHBCI_LOGDOMAIN, "here (%d)", rv);
    GWEN_DB_Group_free(cfg);
    GWEN_Buffer_free(mbuf);
  ...
  GWEN_DB_SetBinValue(cfg, GWEN_DB_FLAGS_DEFAULT, "cryptdata", GWEN_Buffer_GetStart(mbuf), GWEN_Buffer_GetUsedBytes(mbuf));
  GWEN_Buffer_free(mbuf);
...
int AH_Msg_DecryptRdh7(AH_MSG *hmsg, GWEN_DB_NODE *gr){
  AH_HBCI *h;
  GWEN_BUFFER *mbuf;
  uint32_t l;
  int rv;
  ...
  uint8_t decKey[AH_MSGRDH7_MAXKEYBUF+64];
  ...
  if (p && l) {
    ...
    assert(ksize<=AH_MSGRDH7_MAXKEYBUF);
...
int AH_Msg_VerifyRdh7(AH_MSG *hmsg, GWEN_DB_NODE *gr) {
  AH_HBCI *h;
  GWEN_LIST *sigheads;
  GWEN_LIST *sigtails;
  GWEN_DB_NODE *n;
  ...
  DBG_INFO(AQHBCI_LOGDOMAIN, "===> hm: called AH_Msg_VerifyRdh7 - hmsg, gr");
  ...
  for (i=0; i< GWEN_List_GetSize(sigtails); i++) {
    ...
    /* :TODO: check with public sign key from AH_USER instead of using a key
     * from the crypt token....
     */
    if (1) {
      GWEN_MDIGEST *md;
      uint8_t hash1[32];
    ...
    if (p && l) {
      ...
      rv=AH_MsgRdh7__Verify_Internal(bankPubSignKey, algo,
				     hash, 32, p, l);
      GWEN_Crypt_PaddAlgo_free(algo);
      if (rv) {
  ...
  GWEN_List_free(sigheads);
  GWEN_List_free(sigtails);
  DBG_INFO(AQHBCI_LOGDOMAIN, "===> hm: successfully verified in AH_Msg_VerifyRdh7");
  return 0;
}
