...
int AH_MsgRdh_PrepareCryptoSeg3(AH_MSG *hmsg,
				AB_USER *u,
  ...
  if (crypt) {
    GWEN_DB_SetIntValue(cfg, GWEN_DB_FLAGS_DEFAULT, "cryptAlgo/algo", 13); /* 2-key-DES */
    GWEN_DB_SetIntValue(cfg, GWEN_DB_FLAGS_DEFAULT, "cryptAlgo/mode", 18); /* PKCS#1 */
  }
  else {
    GWEN_DB_SetIntValue(cfg, GWEN_DB_FLAGS_DEFAULT, "signAlgo/algo", 10); /* RSA */
    if (AH_Msg_GetSecurityClass(hmsg)<3) {
      /* S-key */
      GWEN_DB_SetIntValue(cfg, GWEN_DB_FLAGS_DEFAULT, "signAlgo/mode", 18); /* PKCS#1 */
      GWEN_DB_SetIntValue(cfg, GWEN_DB_FLAGS_DEFAULT, "hashAlgo/algo", 1); /* SHA-1 */
    }
    else {
      /* D-key */
...
int AH_Msg_SignRdh3(AH_MSG *hmsg,
                    AB_USER *su,
		    GWEN_BUFFER *rawBuf,
		    const char *signer) {
  AH_HBCI *h;
  ...
  rv=AH_MsgRdh_PrepareCryptoSeg3(hmsg, su, ki, cfg, 0, 1);
  if (rv) {
...
int AH_Msg_EncryptRdh3(AH_MSG *hmsg) {
  AH_HBCI *h;
  GWEN_XMLNODE *node;
  GWEN_DB_NODE *cfg;
  GWEN_BUFFER *mbuf;
...
//  uFlags=AH_User_GetFlags(u);
  ...
  rv=AH_MsgRdh_PrepareCryptoSeg3(hmsg, u, ki, cfg, 1, 0);
  if (rv) {
    DBG_INFO(AQHBCI_LOGDOMAIN, "here (%d)", rv);
    GWEN_DB_Group_free(cfg);
    GWEN_Buffer_free(mbuf);
...
int AH_Msg_DecryptRdh3(AH_MSG *hmsg, GWEN_DB_NODE *gr){
  AH_HBCI *h;
  GWEN_BUFFER *mbuf;
  uint32_t l;
  int rv;
...
int AH_Msg_VerifyRdh3(AH_MSG *hmsg, GWEN_DB_NODE *gr) {
  AH_HBCI *h;
  GWEN_LIST *sigheads;
  GWEN_LIST *sigtails;
  GWEN_DB_NODE *n;
