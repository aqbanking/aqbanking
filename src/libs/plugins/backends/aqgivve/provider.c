    ...
    while ( uq ) {
        ...
        if ( token ) {
            ...
            while ( aq ) {
                ...
                AB_IMEXPORTER_ACCOUNTINFO *ai=AB_ImExporterContext_GetOrAddAccountInfo ( ctx, AB_Account_GetUniqueId ( a ), AB_Account_GetIban ( a ), AB_Account_GetBankCode ( a ), AB_Account_GetAccountNumber ( a ),AB_Account_GetAccountType ( a ) );
...
int AG_Provider_ExecGetTrans ( AB_PROVIDER* pro, AB_IMEXPORTER_ACCOUNTINFO* ai, AB_ACCOUNT* account, AB_TRANSACTION* j, char* token )
{
    ...
    AB_TRANSACTION_LIST *list = AG_Provider_Request_GetTransactions ( account, AB_Transaction_GetFirstDate ( j ), AB_Transaction_GetLastDate ( j ), token );
    AB_TRANSACTION *t;
    if ( list ) {
