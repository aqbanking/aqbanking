...
int AG_HttpSession_InitSyncIo ( GWEN_HTTP_SESSION *sess, GWEN_SYNCIO *sio )
{
    ...
    while ( var ) {
        ...
        GWEN_DB_SetCharValue ( header, GWEN_DB_FLAGS_OVERWRITE_VARS, GWEN_DB_VariableName ( var ), GWEN_DB_GetCharValueFromNode ( val ) );
...
char * AG_Provider_Request_GetToken ( AB_USER* user )
{
    ...
    snprintf ( text, sizeof ( text )-1, "Please enter the password for user %s\n<html>Please enter the password for user <i>%s</i></br></html>", identifier, identifier );

    GWEN_Gui_GetPassword ( 0, pw_token, "Enter Password", text, pw, 4, sizeof ( pw )-1, GWEN_Gui_PasswordMethod_Text, NULL, 0 );

    char request[1024];

    snprintf ( request, 1024, "{\"identifier\": \"%s\", \"password\": \"%s\", \"accessors\": [ \"voucher_owner\"]}", identifier, pw );
...
AB_TRANSACTION_LIST * AG_Provider_Request_GetTransactions ( AB_ACCOUNT *account, const GWEN_DATE *start_date, const GWEN_DATE *end_date, char* token )
{
    ...
    while ( current_page <= total_pages ) {
        ...
        snprintf ( path, sizeof ( path ) - 1,"/api/vouchers/%s/transactions/?page[number]=%d&filter[status][$in]=Settled%s%s", id, current_page, filter_start, filter_end );
