...
int transfers(AB_BANKING *ab,
...
#ifdef WITH_AQFINANCE
  {
    ...
    0, 
    "usedb",
    "store transfers in internal database",
    "store transfers in internal database"
  },
#endif
  {
...
#ifdef WITH_AQFINANCE
  if (useDb) {
    rv=AFM_add_transfers(ab, tlist, AE_Statement_StatusSending);
    if (rv<0) {
      DBG_ERROR(0, "Unable to add transfers to internal DB, aborting");
      AB_Transaction_List2_free(tlist);
      AB_ImExporterContext_free(ctx);
      GWEN_DB_Group_free(dbProfiles);
      return 3;
