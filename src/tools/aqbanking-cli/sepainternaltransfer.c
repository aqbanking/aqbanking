...
int sepaInternalTransfer(AB_BANKING *ab, GWEN_DB_NODE *dbArgs, int argc, char **argv)
{
  ...
  /* transfer arguments */
  {
      ...
      AB_REFERENCE_ACCOUNT *ra=AB_ReferenceAccount_List_FindFirst(ral,iban,NULL,NULL,NULL,NULL,NULL,NULL,refAccountName);
      if (ra == NULL)
      {
        AB_Banking_Fini(ab);
        return 2;
      ...
      GWEN_DB_SetCharValue(db,GWEN_DB_FLAGS_OVERWRITE_VARS,"remoteIban",AB_ReferenceAccount_GetIban(ra));
      GWEN_DB_SetCharValue(db,GWEN_DB_FLAGS_OVERWRITE_VARS,"remoteBic",AB_ReferenceAccount_GetBic(ra));
      GWEN_DB_SetCharValue(db,GWEN_DB_FLAGS_OVERWRITE_VARS,"remoteName",AB_ReferenceAccount_GetOwnerName(ra));
  }
