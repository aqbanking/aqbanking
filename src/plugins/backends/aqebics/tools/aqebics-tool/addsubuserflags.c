...
int addSubUserFlags(AB_BANKING *ab,
  ...
  ul=AB_Banking_FindUsers(ab, EBC_PROVIDER_NAME, "de",
  ...
  if (!u) {
    ...
    if (doAdd) {
      ...
      fprintf(stderr, "Removingg flags: %08x\n", flags);
...
int addUserFlags(AB_BANKING *ab,
		 ...
		 int argc,
		 char **argv) {
  return addSubUserFlags(ab, dbArgs, argc, argv, 1);
}
...
int subUserFlags(AB_BANKING *ab,
		 ...
		 int argc,
		 char **argv) {
  return addSubUserFlags(ab, dbArgs, argc, argv, 0);
}
