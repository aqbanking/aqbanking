...
GWEN_INHERIT(AH_JOB, AH_JOB_SEPAXFERSINGLE);
...
AH_JOB *AH_Job_SepaTransferSingle_new(AB_USER *u, AB_ACCOUNT *account) {
  ...
  GWEN_NEW_OBJECT(AH_JOB_SEPAXFERSINGLE, aj);
  GWEN_INHERIT_SETDATA(AH_JOB, AH_JOB_SEPAXFERSINGLE, j, aj,
                       AH_Job_SepaTransferSingle_FreeData);

  /* overwrite some virtual functions */
  AH_Job_SetExchangeFn(j, AH_Job_SepaTransferSingle_Exchange);
  AH_Job_SetProcessFn(j, AH_Job_SepaTransferSingle_Process);
...
/* --------------------------------------------------------------- FUNCTION */
void GWENHYWFAR_CB AH_Job_SepaTransferSingle_FreeData(void *bp, void *p){
  AH_JOB_SEPAXFERSINGLE *aj;

  aj=(AH_JOB_SEPAXFERSINGLE*)p;
...
int AH_Job_SepaTransferSingle_ExchangeParams(AH_JOB *j, AB_JOB *bj,
  ...
  assert(j);
  aj=GWEN_INHERIT_GETDATA(AH_JOB, AH_JOB_SEPAXFERSINGLE, j);
  assert(aj);
...
/* --------------------------------------------------------------- FUNCTION */
int AH_Job_SepaTransferSingle_ExchangeArgs(AH_JOB *j, AB_JOB *bj,
                                           AB_IMEXPORTER_CONTEXT *ctx) {
  AH_JOB_SEPAXFERSINGLE *aj;
  GWEN_DB_NODE *dbArgs;
...
/* --------------------------------------------------------------- FUNCTION */
int AH_Job_SepaTransferSingle_ExchangeResults(AH_JOB *j, AB_JOB *bj,
                                              AB_IMEXPORTER_CONTEXT *ctx) {
  AH_JOB_SEPAXFERSINGLE *aj;
  AH_RESULT_LIST *rl;
  AH_RESULT *r;
...
/* --------------------------------------------------------------- FUNCTION */
int AH_Job_SepaTransferSingle_Exchange(AH_JOB *j, AB_JOB *bj,
  ...
  if (AB_Job_GetType(bj)!=AB_Job_TypeSepaTransfer) {
    DBG_ERROR(AQHBCI_LOGDOMAIN, "Not a SepaTransfer job");
    return GWEN_ERROR_INVALID;
  }

  switch(m) {
  case AH_Job_ExchangeModeParams:
    return AH_Job_SepaTransferSingle_ExchangeParams(j, bj, ctx);
  case AH_Job_ExchangeModeArgs:
    return AH_Job_SepaTransferSingle_ExchangeArgs(j, bj, ctx);
  case AH_Job_ExchangeModeResults:
    return AH_Job_SepaTransferSingle_ExchangeResults(j, bj, ctx);
  default:
...
/* --------------------------------------------------------------- FUNCTION */
int AH_Job_SepaTransferSingle_Process(AH_JOB *j, AB_IMEXPORTER_CONTEXT *ctx) {
  AH_JOB_SEPAXFERSINGLE *aj;
