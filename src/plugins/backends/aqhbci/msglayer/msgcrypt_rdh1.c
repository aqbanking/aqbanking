...
int AH_MsgRdh_PrepareCryptoSeg1(AH_MSG *hmsg,
				AB_USER *u,
...
int AH_Msg_SignRdh1(AH_MSG *hmsg,
                    AB_USER *su,
		    GWEN_BUFFER *rawBuf,
		    const char *signer) {
  AH_HBCI *h;
  ...
  rv=AH_MsgRdh_PrepareCryptoSeg1(hmsg, su, ki, cfg, 0, 1);
  if (rv) {
  ...
  if (1) {
    ...
    GWEN_Crypt_PaddAlgo_SetPaddSize(algo, 96);
    signLen=96;
...
int AH_Msg_EncryptRdh1(AH_MSG *hmsg) {
  AH_HBCI *h;
  GWEN_XMLNODE *node;
  GWEN_DB_NODE *cfg;
  GWEN_BUFFER *mbuf;
...
//  uFlags=AH_User_GetFlags(u);
  ...
  rv=AH_MsgRdh_PrepareCryptoSeg1(hmsg, u, ki, cfg, 1, 0);
  if (rv) {
    DBG_INFO(AQHBCI_LOGDOMAIN, "here (%d)", rv);
    GWEN_DB_Group_free(cfg);
    GWEN_Buffer_free(mbuf);
...
int AH_Msg_DecryptRdh1(AH_MSG *hmsg, GWEN_DB_NODE *gr){
  AH_HBCI *h;
  GWEN_BUFFER *mbuf;
  uint32_t l;
  int rv;
...
int AH_Msg_VerifyRdh1(AH_MSG *hmsg, GWEN_DB_NODE *gr) {
  AH_HBCI *h;
  GWEN_LIST *sigheads;
  GWEN_LIST *sigtails;
  GWEN_DB_NODE *n;
