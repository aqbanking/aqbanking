...
int CBanking__ConvertFromUtf8(AB_BANKING *ab,
  ...
  if (cb->charSet) {
    if (strcasecmp(cb->charSet, "utf-8")!=0) {
#ifndef HAVE_ICONV_H
      DBG_INFO(0, "iconv not available, can not convert to \"%s\"",
               cb->charSet);
#else
      iconv_t ic;
      ...
      if (ic==((iconv_t)-1)) {
        DBG_ERROR(0, "Charset \"%s\" not available",
                  cb->charSet);
      }
      else {
        char *outbuf;
...
void CBanking_GetRawText(AB_BANKING *ab,
  ...
  if (rv) {
    DBG_ERROR(0, "Error converting text");
...
int CBanking_InputBox(AB_BANKING *ab,
  ...
  if (flags & AB_BANKING_INPUT_FLAGS_CONFIRM) {
    for (;;) {
      ...
      if (!lbuffer) {
        DBG_ERROR(0, "Not enough memory for %d bytes", maxLen);
...
int CBanking_ProgressAdvance(AB_BANKING *ab,
  ...
  if (!pr) {
    DBG_INFO(0, "Progress \"%d\" not found", id);
    return AB_ERROR_INVALID;
  }
...
int CBanking_ProgressEnd(AB_BANKING *ab, GWEN_TYPE_UINT32 id){
  ...
  pr=CBanking__findProgress(ab, id);
  if (!pr) {
    DBG_ERROR(0, "Progress \"%d\" not found", id);
    return AB_ERROR_INVALID;
  }
...
int CBanking_GetPin(AB_BANKING *ab,
  ...
  if (cb->dbPins) {
    ...
    if (GWEN_Text_EscapeToBuffer(token, nbuf)) {
      DBG_ERROR(0, "Error escaping token name");
      return -1;
    }
