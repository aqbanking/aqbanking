...
AB_PROVIDER *AD_Provider_new(AB_BANKING *ab){
  ...
  pro=AB_Provider_new(ab, "aqdtaus");
  ...
  AB_Provider_SetGetAccountListFn(pro, AD_Provider_GetAccountList);
  AB_Provider_SetUpdateAccountFn(pro, AD_Provider_UpdateAccount);
...
int AD_Provider_Init(AB_PROVIDER *pro, GWEN_DB_NODE *dbData) {
  ...
  while(dbAccount) {
    ...
    da=AD_Account_fromDb(AB_Provider_GetBanking(pro), dbAccount);
    if (!da) {
      DBG_ERROR(AQDTAUS_LOGDOMAIN, "Error loading account");
    }
    else {
      DBG_INFO(AQDTAUS_LOGDOMAIN, "Adding account");
  ...
  if (ait) {
    ...
    while(da) {
      ...
      dbAccount=GWEN_DB_GetGroup(dbAccounts, GWEN_PATH_FLAGS_CREATE_GROUP,
                                 "account");
      rv=AD_Account_toDb(da, dbAccount);
      if (rv) {
        DBG_ERROR(AQDTAUS_LOGDOMAIN, "Error saving account");
        errors++;
      }
...
AB_ACCOUNT *AD_Provider_FindMyAccount(AB_PROVIDER *pro,
  ...
  if (accountNumber==0 || bankCode==0) {
    DBG_INFO(AQDTAUS_LOGDOMAIN, "Invalid banking account");
    return 0;
  }
  ...
  if (ait) {
    ...
    while(da) {
      ...
      lAccountNumber=AB_Account_GetAccountNumber(acc);
      lBankCode=AB_Account_GetBankCode(acc);
      if (lAccountNumber==0 || lBankCode==0) {
        DBG_INFO(AQDTAUS_LOGDOMAIN, "Invalid backend account");
      }
      else {
  ...
  DBG_INFO(AQDTAUS_LOGDOMAIN, "Backend account not found");
  return 0;
}
...
int AD_Provider_UpdateJob(AB_PROVIDER *pro, AB_JOB *j){
  ...
  da=AD_Provider_FindMyAccount(pro, AB_Job_GetAccount(j));
  if (!da) {
    DBG_ERROR(AQDTAUS_LOGDOMAIN, "Account not managed by this backend");
...
int AD_Provider_UpdateAccount(AB_PROVIDER *pro, AB_ACCOUNT *a){
  ...
  DBG_NOTICE(AQDTAUS_LOGDOMAIN, "TODO: UpdateAccount");
  return 0;
}
