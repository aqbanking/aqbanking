/***************************************************************************
 $RCSfile$
                             -------------------
    cvs         : $Id: context.c 1326 2007-10-26 22:48:34Z martin $
...
AO_CONTEXT *AO_Context_new(AB_USER *u, AB_JOB *job,
  ...
  ctx->fi=(struct OfxFiLogin*)GWEN_Memory_malloc(sizeof(struct OfxFiLogin));
  ctx->ai=(struct OfxAccountData*)GWEN_Memory_malloc(sizeof(struct OfxAccountData));
  assert(ctx->ai);

  memset(ctx->ai, 0, sizeof(struct OfxAccountData));
...
int AO_Context_Update(AO_CONTEXT *ctx, uint32_t guiid){
  ...
  if (ctx->user) {
	...
	strncpy(ctx->ai->bank_id, s, OFX_BANKID_LENGTH-1);
    }

    s=AO_User_GetBrokerId(ctx->user);
    if (s)
      strncpy(ctx->ai->broker_id, s, OFX_BROKERID_LENGTH-1);

    s=AO_User_GetOrg(ctx->user);
    if (s)
      strncpy(ctx->fi->org, s, OFX_ORG_LENGTH-1);

    s=AO_User_GetAppId(ctx->user);
    if (s)
      strncpy(ctx->fi->appid, s, OFX_APPID_LENGTH-1);

    s=AO_User_GetAppVer(ctx->user);
    if (s)
      strncpy(ctx->fi->appver, s, OFX_APPVER_LENGTH-1);

    s=AO_User_GetHeaderVer(ctx->user);
    if (s)
      strncpy(ctx->fi->header_version, s, OFX_HEADERVERSION_LENGTH-1);
  ...
  if (ctx->job) {
    ...
    if (s)
      strncpy(ctx->ai->account_number, s, OFX_ACCTID_LENGTH-1);
...
int AO_Context_StatementCallback(const struct OfxStatementData data,
  ...
  if (data.ledger_balance_valid ||
    ...
    if (data.ledger_balance_valid) {
      ...
      v=AB_Value_fromDouble(data.ledger_balance);
      AB_Value_SetCurrency(v, currency);
    ...
    if (data.available_balance_valid) {
      ...
      v=AB_Value_fromDouble(data.available_balance);
      AB_Value_SetCurrency(v, currency);
