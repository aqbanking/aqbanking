...
int AIO_OfxGroup_INVACC_StartTag(AIO_OFX_GROUP *g, const char *tagName) {
  ...
  if (strcasecmp(tagName, "BANKID")==0 ||
      strcasecmp(tagName, "ACCTID")==0)  xg->currentElement=strdup(tagName);
  else DBG_WARN(AQBANKING_LOGDOMAIN, "Ignoring tag [%s]", tagName);
  return 0;
}
...
int AIO_OfxGroup_INVACC_AddData(AIO_OFX_GROUP *g, const char *data) {
  ...
  if (xg->currentElement) {
    ...
    if (*s) {
      ...
      if (strcasecmp(xg->currentElement, "BROKERID")==0) AIO_OfxGroup_INVACC_SetBrokerId(g, GWEN_Buffer_GetStart(buf));
      else if (strcasecmp(xg->currentElement, "ACCTID")==0) AIO_OfxGroup_INVACC_SetAccId(g, GWEN_Buffer_GetStart(buf));
      else DBG_INFO(AQBANKING_LOGDOMAIN, "Ignoring data for unknown element [%s]", xg->currentElement);
    }
    GWEN_Buffer_free(buf);
  }
  return 0;
