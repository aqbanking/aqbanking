...
char * INCOME_DataTags[iincomelastdatum]={"TOTAL", "INCOMETYPE", "SUBACCTSEC", "SUBACCTFUND"};
char * INCOME_GroupTags[iincomelastgroup]={"SECID","INVTRAN"};
...
int AIO_OfxGroup_INCOME_StartTag(AIO_OFX_GROUP *g, const char *tagName) {
  ...
  if(ct<0) {
    ...
    if (ct<0) {
      ...
      if (ct==iincomesecid) gNew=AIO_OfxGroup_SECID_new(tagName, g, ctx);
      else if (ct==iincomeinvtran) gNew=AIO_OfxGroup_INVTRAN_new(tagName, g, ctx);
    }
    if (gNew) {
...
int AIO_OfxGroup_INCOME_AddData(AIO_OFX_GROUP *g, const char *data) {
  ...
  xg=GWEN_INHERIT_GETDATA(AIO_OFX_GROUP, AIO_OFX_GROUP_INCOME, g);
  assert(xg);
  if (!xg->currentElement) return 0;      /*No element - just ignore these*/
...
/*Fetch a copy of this groups context - and it better be valid too.*/
  ...
  if (eg<0) return 0;         /*Not a recognised group*/
...
int AIO_OfxGroup_INCOME_SortTag(const char * s, char ** sTags, int max){
  int i;
  for (i=0; i<max; i++)
